...
| 统计 | 心情分布图 + 日历热力图(可点击下钻) + 心情速览条 | Bearer |
| 时光胶囊 | 定时解锁日记 (到期前内容屏蔽) | Bearer |
| 树洞 | 随机漂流瓶 + 抱抱 + 匿名回复 + 上一个/下一个翻页 + 作者标签 | 无 |
| 公开广场 | 心情/标签/关键词搜索（支持中文心情词+昵称+AI字段）+ 分页 + 点亮 + 评论 + 作者标签 | 可选 |
| 关注系统 | 关注/取关 + 关注动态 + 关注/粉丝列表 | Bearer |
| 用户主页 | 个人资料编辑 + 公开主页 + 关注统计 | Bearer/可选 |
| 通知中心 | 实时通知 + 已读/未读 + 批量操作 | Bearer |
| 打招呼 | 发起/同意/拒绝/取消 + 状态查询 | Bearer |
| 私信 | 会话列表 + 发送消息/图片 + 历史记录 + 已读标记 | Bearer |
...
> **新用户注册**：头像从 18 个 emoji 池中随机选取（🐰🐱🐶🐻🦊🐼🐨🐙🦁🐮🐸🐵🦄🐳🌸⭐🌈🍀），注册后可在个人资料中修改。

## 最新功能

| 功能 | 说明 |
|------|------|
| 🌳 树洞翻页 | "上一个/下一个"按钮浏览历史漂流瓶，手滑也能找回 |
| 📷 私信图片 | 私信支持发送图片消息 |
...
> 截图路径: `docs/screenshots/`（手动截图后放入对应文件）

| 界面 | 说明 | 截图 |
|------|------|------|
| 日记时间线 | 日记列表 + 日历下钻 + 心情速览 + 搜索 | `timeline.png` |
| 写日记 | 心情选择(6种) + AI分析 + 多图上传 | `write.png` |
| 发现广场 | 公开日记 + 心情/标签/关键词筛选 + 评论 | `discover.png` |
| 日记详情 | 公开日记详情 + 评论(含作者标签) | `detail.png` |
| 树洞 | 随机漂流瓶 + 匿名回复(含作者标签) | `treehole.png` |
| 我的主页 | 个人资料 + 统计看板 + 时光胶囊 | `profile.png` |
| 消息中心 | 私信 + 打招呼 + 通知(含未读角标) | `messages.png` |
| 关注列表 | 关注/粉丝 + 发消息入口 | `follow.png` |
| 安全中心 | 拉黑列表 + 举报记录 | `safety.png` |
